...
/*
 ...
 - 특정한 값을 Search 하는데 데이터 고유의 인덱스 (키) 로 접근하게 되므로 average case 에 대하여 Time Complexity 가 O(1)이 된다.
 - 항상 O(1)이 아니고 average case 에 대해서 O(1)인 것은 collision 때문이다.
 - 이 인덱스로 저장되는 키 값이 불규칙하다는 것이 문제점이 될 수 있다.

 ## Hash 함수란?

 - 해시 함수는 임의의 길이를 갖는 input 을 입력 받아 고정된 길이의 해시값을 출력하는 함수
 - 특징
     - 결과 값을 바탕으로 입력 값을 유추할 수 없음
     - 입력값이 하나만 바뀌어도 결과값이 완전히 달라지는 눈사태 효과가 나타난다.
 
 
 
 ## Hash Collision (해시 충돌)

 - 특정 key 값을 변환하는 과정에서 서로 다른 key 값이 같은 hash value 로 변경되는 문제
 - 예를 들어 해시함수 h(x) 가 7로 나눈 나머지 값이라 한다면?
     - 8개 이상의 값이 들어오면 무조건 같은 값이 1개 존재할 수밖에 없다!
 - 즉, 서로 다른 두 개의 키가 같은 인덱스로 hashing(hash 함수를 통해 계산됨을 의미)되면 같은 곳에 저장할 수 없게 된다.
 - 따라서 hashing 된 인덱스에 이미 다른 값이 들어 있다면 새 데이터를 저장할 다른 위치를 찾은 뒤에야 저장할 수 있는 것이다.
 ...
 - Java 7 에서는 Separate Chaining 방식을 사용하여 HashMap 을 구현하고 있다. Separate Chaining 방식으로는 두 가지 구현 방식이 존재한다.
     ...
     - **Tree 를 사용하는 방식 (Red-Black Tree)**
         - 기본적인 알고리즘은 Separate Chaining 방식과 동일하며 연결 리스트 대신 트리를 사용하는 방식이다. 연결 리스트를 사용할 것인가와 트리를 사용할 것인가에 대한 기준은 하나의 해시 버킷에 할당된 key-value 쌍의 개수이다.
